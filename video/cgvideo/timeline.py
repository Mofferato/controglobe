...
def write_resolve_lua(cfg: dict, slots: list[Slot], meta: dict) -> None:
    ...
    free edition, in a sandbox without the `io` library, so the script cannot write a log
    file there. It reports in a Resolve window instead (text you can select and copy), and
    in Workspace > Console. If the edition refuses scripts the project API, the report
    carries the manual import, which needs no scripting. Copy this file to
    ...
    edl = (P.output / "markers.edl").resolve().as_posix()
    ...
    manual = "\n".join([
        ...
        f"2. Media page: drag the folder {seq_dir} into the Media Pool. It arrives as one clip.",
        "3. Right-click the clip > Create New Timeline Using Selected Clips.",
        "4. Right-click the new timeline in the Media Pool > Timelines > Import >",
    ...
    lines = [
        ...
        "-- with a project open. It reports in a window (select the text to copy it) and in",
        "-- Workspace > Console. The free edition's Lua has no io library: no log file there.",
        ...
        "    win:Hide()",
        "  end)",
        '  if not ok then print("(Resolve offered no dialog; the report is above in this Console.)") end',
        "end",
        "",
        ...
        "  -- A menu script may be handed a live `resolve`; else ask for one. On free 21.1,",
        "  -- Resolve() exists but returns nil: the edition keeps project scripting for Studio.",
        "  local r = resolve",
        ...
        "  if not r then",
        '    say("RESULT: this DaVinci Resolve does not let scripts control projects.")',
        '    say("The free edition from 21.1 on keeps that for Studio. Nothing was changed.")',
        '    say("")',
        "    say(MANUAL)",
        "    return",
        "  end",
        ...
        f"  local items = mp:ImportMedia({{{{FilePath = {_lua_str(seq)}, StartIndex = 1, EndIndex = {meta['total_frames']}}}}})",
        ...
        "  end",
        "  if not items or #items == 0 then",
        '    say("RESULT: the frames did not import. Run `python build.py sequence` first.")',
        "    return",
        "  end",
        ...
        '  say("Timeline: " .. tl:GetName() .. ", " .. (tl:GetEndFrame() - tl:GetStartFrame()) .. " frames.")',
        "  local marks = {",
    ]
    ...
    write_marker_edl(cfg, marks, meta)
...
def write_marker_edl(cfg: dict, marks: list[tuple], meta: dict) -> None:
    ...
    for n, (f, color, name, note, dur) in enumerate(marks, 1):
        ...
        out.append("")
    (paths(cfg).output / "markers.edl").write_text("\n".join(out), encoding="utf8")
