...
def write_resolve_lua(cfg: dict, slots: list[Slot], meta: dict) -> None:
    ...
    lines = [
        ...
        "-- Everything it does is written to the log below, as well as to Workspace > Console.",
        ...
        "",
        "local function main()",
        "  -- A script run from Workspace > Scripts is handed a live `resolve`; Resolve() is the",
        "  -- console's spelling and returns nil there on the free edition, so try it last.",
        '  say("globals: resolve=" .. type(resolve) .. " Resolve=" .. type(Resolve) .. " bmd=" .. type(bmd)',
        '      .. " app=" .. type(app) .. " fu=" .. type(fu))',
        "  local r = resolve",
        '  if not r and bmd and bmd.scriptapp then r = bmd.scriptapp("Resolve") end',
        "  if not r and app and app.GetResolve then r = app:GetResolve() end",
        "  if not r and Resolve then r = Resolve() end",
        '  if not r then say("FAILED: no Resolve scripting object. This Resolve build does not let scripts drive it.") return end',
        "  local project = r:GetProjectManager():GetCurrentProject()",
        '  if not project then say("FAILED: no project open. Open or create a project, then run this again.") return end',
        ...
        '    say("ImportMedia with a frame pattern returned nothing; importing the folder instead")',
        f"    items = r:GetMediaStorage():AddItemListToMediaPool({{{_lua_str(seq_dir)}}})",
        "  end",
        '  if not items or #items == 0 then say("FAILED: could not import the sequence. Run `python build.py sequence` first.") return end',
        '  say("imported " .. #items .. " clip(s)")',
        '  local tl = mp:CreateTimelineFromClips("Controglobe v1", items)',
        '  if not tl then say("FAILED: CreateTimelineFromClips returned nothing") return end',
